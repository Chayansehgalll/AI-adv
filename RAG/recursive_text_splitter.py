...
tesla_text = f"""Tesla's Q3 Results
...
# splitter1 = CharacterTextSplitter( # Basic CharacterTextSplitter has issues with long paragraphs that don't have double newlines
#     separator=" ",  # Default separator. Other options include ["\n\n", "\n", ". ", " ", ""]
#     chunk_size=100, # can only split on one seperator, we can't split on multiple seperators, 
#     chunk_overlap=0                 so if we have a long paragraph with no double newlines, it will not split properly
# )
...
recursive_splitter = RecursiveCharacterTextSplitter(
    separators=["\n\n", "\n", ". ", " ", ""],  # Multiple separators
    chunk_size=100,
    chunk_overlap=0 # it means that the chunks will not overlap, so if we have a long paragraph with no double newlines, it will still split properly
)
